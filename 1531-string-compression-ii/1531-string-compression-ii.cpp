//dp of below size cannot be created inside functions and if we try to make it, the program will give runtime error, it can only be created as a global array.
...
public:
    int getLengthOfOptimalCompression(string s, int k) {
        ...
        int size = s.size() + 1;
        //Initializing all values of dp as -1.
        ...
        function<int(int,int,int,int)> helper = [&](int cur_pos, int left_k, int last_char, int last_count) -> int{//( -> int means that integer is the return type of above lambda function).
